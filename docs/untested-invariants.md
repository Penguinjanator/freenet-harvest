...
| `harvest_common::mailbox::enforce_message_cap` and `apply_delta`'s final sort | Each imposes a total order by itself. | **No, and deliberately so.** Both gained a digest tiebreak when identity moved to `entry_digest`; removing EITHER leaves all 477 tests green, because `dedupe_identical_entries` sorts the whole collection by digest first and both later sorts are stable. They are self-sufficiency, not the mechanism -- and both comments now say that rather than claiming the property. Convergence itself IS pinned, at the sort that carries it: `two_different_messages_sharing_a_nonce_converge_and_both_survive` fails when dedup is made order-dependent. Found by mutating my own new code and getting green, which is the H3 lesson arriving one commit later. |
| `harvest_common::mailbox::dedupe_by_nonce` | The counterparty can delete a message you sent, and no tiebreak can stop it. | **Pinned as a LIMITATION** by the tests above, which demonstrate the deletion through the real contract. The *reason* no tiebreak helps -- a pure function of the SET cannot know which arrived first -- is an argument, not a test, and is written up in `docs/messaging-privacy.md`, along with the correction of a first attempt at a fix (a deterministic nonce) that would not have worked because the nonce is a field the writer fills in and nothing can enforce a derivation rule against a key holder. |
