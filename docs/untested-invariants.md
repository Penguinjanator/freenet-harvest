...
| `harvest_common::mailbox::enforce_message_cap` | The mailbox is bounded in bytes, and two peers prune to byte-identical state. | **Yes** -- `the_mailbox_is_bounded_in_bytes_and_not_only_in_count` (red against the count-only predecessor) and `merging_is_order_independent_across_the_byte_budget` (red against a rule that sorted only when the COUNT cap bound). Note the batch-independence test did NOT catch that mutation: chunks arrive in the same relative order, so the two convergence tests do not bite equally and only the order one covers the ranking. |
